...
ArrangePanel.prototype.addLayerOps = function (div) {
    ...
    let toFrontBtn = mxUtils.button(mxResources.get('toFront'), function () {
        ui.actions.get('toFront').funct();
    });

    toFrontBtn.setAttribute('title', mxResources.get('toFront') + ' (' + this.editorUi.actions.get('toFront').shortcut + ')');
    toFrontBtn.style.width = '100px';
    toFrontBtn.style.marginRight = '2px';
    div.appendChild(toFrontBtn);

    let toBackBtn = mxUtils.button(mxResources.get('toBack'), function () {
        ui.actions.get('toBack').funct();
    });

    toBackBtn.setAttribute('title', mxResources.get('toBack') + ' (' + this.editorUi.actions.get('toBack').shortcut + ')');
    toBackBtn.style.width = '100px';
    div.appendChild(toBackBtn);
...
ArrangePanel.prototype.addGroupOps = function (div) {
    ...
    if (ss.vertices.length > 0) {
        ...
        let btn = mxUtils.button(mxResources.get('copySize'), function () {
        ...
        if (ui.copiedSize != null) {
            let btn2 = mxUtils.button(mxResources.get('pasteSize'), function () {
    ...
    if (graph.getSelectionCount() === 1 && graph.getModel().isVertex(cell) &&
        ...
        btn = mxUtils.button(mxResources.get('clearWaypoints'), mxUtils.bind(this, function () {
    ...
    if (graph.getSelectionCount() === 1) {
        ...
        btn = mxUtils.button(mxResources.get('editData'), mxUtils.bind(this, function () {
        ...
        btn = mxUtils.button(mxResources.get('editLink'), mxUtils.bind(this, function () {
...
ArrangePanel.prototype.addFlip = function (div) {
    ...
    let horizontalBtn = mxUtils.button(mxResources.get('horizontal'), function () {
        graph.toggleCellStyles(mxConstants.STYLE_FLIPH, false);
    });

    horizontalBtn.setAttribute('title', mxResources.get('horizontal'));
    horizontalBtn.style.width = '100px';
    horizontalBtn.style.marginRight = '2px';
    div.appendChild(horizontalBtn);

    let verticalBtn = mxUtils.button(mxResources.get('vertical'), function () {
        graph.toggleCellStyles(mxConstants.STYLE_FLIPV, false);
    });

    verticalBtn.setAttribute('title', mxResources.get('vertical'));
    verticalBtn.style.width = '100px';
    div.appendChild(verticalBtn);
...
ArrangePanel.prototype.addDistribute = function (div) {
    ...
    let btn = mxUtils.button(mxResources.get('vertical'), function () {
...
ArrangePanel.prototype.addAngle = function (div) {
    ...
    if (!ss.containsLabel) {
        ...
        btn = mxUtils.button(label, function () {
...
ArrangePanel.prototype.addGeometry = function (container) {
    ...
    mxUtils.write(span2, mxResources.get('position'));
...
ArrangePanel.prototype.addEdgeGeometry = function (container) {
    ...
    let xtUpdate, ytUpdate, xsUpdate, ysUpdate;
    ...
    mxUtils.write(span3, 'End');
    divt.appendChild(span3);
...
TextFormatPanel.prototype.addFont = function (container) {
    ...
    let fontMenu = this.editorUi.toolbar.addMenu('Helvetica', mxResources.get('fontFamily'), true, 'fontFamily', stylePanel);
    ...
    if (graph.cellEditor.isContentEditing()) {
        let clear = this.editorUi.toolbar.addButton('geSprite-removeformat', mxResources.get('removeFormat'),
            function () {
    ...
    if (!graph.cellEditor.isContentEditing()) {
        ...
        container.appendChild(this.createRelativeOption(mxResources.get('lineheight'), null, null, function (input) {
            ...
            let node = graph.getSelectedElement();
        ...
        this.styleButtons(horizontalBtns);
        ...
        this.styleButtons(tableBtns);
        tableBtns[2].style.marginRight = '9px';
            ...
            if (!updating) {
                ...
                window.setTimeout(function () {
                    ...
                    if (node != null) {
                        ...
                        let lineHeight = getRelativeLineHeight(fontSize, css.lineHeight, node);
                        ...
                        if (elts.length > 0 && window.getSelection && !mxClient.IS_IE && !mxClient.IS_IE11) {
                            ...
                            for (let i = 0; i < elts.length; i++) {
                                if (selection.containsNode(elts[i], true)) {
                                    ...
                                    let lh = getRelativeLineHeight(fontSize, temp.lineHeight, elts[i]);
...
StyleFormatPanel.prototype.addEditOps = function (div) {
    ...
    if (ss.image) {
        let btn2 = mxUtils.button(mxResources.get('editImage'), mxUtils.bind(this, function (evt) {
...
StyleFormatPanel.prototype.addFill = function (container) {
    ...
    let gradientPanel = this.createCellColorOption(mxResources.get('gradient'), mxConstants.STYLE_GRADIENTCOLOR, '#ffffff', function (color) {
        if (color == null || color === mxConstants.NONE) {
    ...
    let fillPanel = this.createCellColorOption(label, fillKey, '#ffffff');
...
StyleFormatPanel.prototype.addStroke = function (container) {
    ...
    let lineColor = this.createCellColorOption(label, strokeKey, '#000000');
    ...
    span2.style.marginBottom = '12px';
    span2.style.marginTop = '1px';
    span2.style.fontWeight = 'normal';
    span2.style.width = '120px';
    mxUtils.write(span2, mxResources.get('perimeter'));
    perimeterPanel.appendChild(span2);
