...
Actions.prototype.init = function () {
        ...
        if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
            ui.showLinkDialog('', mxResources.get('insert'), function (link, docs) {
                ...
                if (link.length > 0) {
                    ...
                    let linkCell = new mxCell(title, new mxGeometry(pt.x, pt.y, 100, 40),
    ...
    this.addAction('toggleRounded', function () {
        if (!graph.isSelectionEmpty() && graph.isEnabled()) {
            ...
            try {
                ...
                let style = (state != null) ? state.style : graph.getCellStyle(cells[0]);
    ...
    this.addAction('subscript', mxUtils.bind(this, function () {
        if (graph.cellEditor.isContentEditing()) {
            document.execCommand('subscript', false, null);
        }
    }), null, null, Editor.ctrlKey + '+,');
    this.addAction('superscript', mxUtils.bind(this, function () {
        if (graph.cellEditor.isContentEditing()) {
