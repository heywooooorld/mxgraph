...
function Graph(container, model, renderHint, stylesheet, themes) {
    ...
    if (typeof mxVertexHandler !== 'undefined') {
        ...
        this.graphHandler.getCells = function (initialCell) {
            let cells = mxGraphHandler.prototype.getCells.apply(this, arguments);
            let newCells = [];

            for (let i = 0; i < cells.length; i++) {
                let state = this.graph.view.getState(cells[i]);
                let style = (state != null) ? state.style : this.graph.getCellStyle(cells[i]);
        ...
        this.selectRegion = function (rect, evt) {
            let cells = this.getAllCells(rect.x, rect.y, rect.width, rect.height);
...
(function () {
    ...
    Graph.prototype.updateCssTransform = function () {
        ...
        if (temp != null) {
            let g = this.view.getDrawPane().parentNode;
...
Graph.prototype.updatePlaceholders = function () {
    ...
    for (let key in this.model.cells) {
...
Graph.prototype.replacePlaceholders = function (cell, str) {
    ...
    if (str != null) {
        let last = 0;
        let match = this.placeholderPattern.exec(str);
        while (match) {
            let val = match[0];
...
Graph.prototype.connectVertex = function (source, direction, length, evt, forceClone, ignoreCellAt) {
    ...
    let target = (ignoreCellAt || (mxEvent.isControlDown(evt) && !forceClone)) ?
    ...
    let duplicate = !mxEvent.isShiftDown(evt) || forceClone;
    ...
    try {
        ...
        if (realTarget == null && duplicate) {
            ...
            let geo = this.getCellGeometry(source);

            while (geo != null && geo.relative) {
                cellToClone = this.getModel().getParent(cellToClone);
                geo1 = this.getCellGeometry(cellToClone);
            }

            // Handle consistuents for cloning
            let state = this.view.getState(cellToClone);
            let style = (state != null) ? state.style : this.getCellStyle(cellToClone);

            if (mxUtils.getValue(style, 'part', false)) {
                let tmpParent = this.model.getParent(cellToClone);
            ...
            let geo1 = this.getCellGeometry(realTarget);

            if (geo1 != null) {
                geo1.x = pt.x - geo1.width / 2;
                geo1.y = pt.y - geo1.height / 2;
            }
        }
        ...
        let edge = ((mxEvent.isControlDown(evt) && duplicate) || (target == null && layout != null && layout.constructor === mxStackLayout)) ? null :
...
//			let elbowValue = (direction === mxConstants.DIRECTION_NORTH || direction === mxConstants.DIRECTION_SOUTH) ? 'vertical' : 'horizontal';
...
Graph.prototype.getTooltipForCell = function (cell) {
    ...
    if (mxUtils.isNode(cell.value)) {
        ...
        if (tmp != null) {
            ...
            let ignored = ['label', 'tooltip', 'placeholders', 'placeholder'];
            ...
            for (let i = 0; i < this.state.routedPoints.length; i++) {
                ...
                if (last != null && rpt.type === 1) {
                    ...
                    if (dist > size * size && len > 0) {
                        ...
                        if (dist > size * size) {
                            let p0 = new mxPoint(pt.x - n.x, pt.y - n.y);
                            let p1 = new mxPoint(pt.x + n.x, pt.y + n.y);
                            pts.push(p0);
        ...
        if (state.style != null && typeof(pako) !== 'undefined') {
            ...
            if (shape != null && shape.substring(0, 8) === 'stencil(') {
                try {
                    ...
                    let doc = mxUtils.parseXml(state.view.graph.decompress(stencil));
...
if (typeof mxVertexHandler !== 'undefined') {
    (function () {
        ...
        Graph.prototype.importGraphModel = function (node, dx, dy, crop) {
            ...
            // let targetChildCount = this.model.getChildCount(this.model.getRoot());
            ...
            try {
                ...
                for (let i = 0; i < childCount; i++) {
                    ...
                    if (childCount === 1 && !this.isCellLocked(this.getDefaultParent())) {
                        ...
                        let children = this.model.getChildren(parent);
         ...
         */
        Graph.prototype.getAllConnectionConstraints = function (terminal) {
            if (terminal != null) {
        ...
        Graph.prototype.flipEdge = function (edge) {
            if (edge != null) {
                ...
                let style = (state != null) ? state.style : this.getCellStyle(edge);
        ...
        Graph.prototype.processChange = function (change) {
            ...
            if (change instanceof mxValueChange && change.cell != null &&
                ...
                let desc = this.model.getDescendants(change.cell);
        ...
        Graph.prototype.encodeCells = function (cells) {
            let clones = this.cloneCells(cells);
        ...
        Graph.prototype.getSvg = function (background, scale, border, nocrop, crisp, ignoreSelection, showText, imgExport) {
            ...
            try {
                ...
                let svgCanvas = this.createSvgCanvas(node);
                ...
                let htmlConverter = document.createElement('textarea');

                // Adds simple text fallback for viewers with no support for foreignObjects
                let createAlternateContent = svgCanvas.createAlternateContent;
        ...
        mxCellEditor.prototype.toggleViewMode = function () {
            let state = this.graph.view.getState(this.editingCell);
            let nl2Br = state != null && mxUtils.getValue(state.style, 'nl2Br', '1') !== '0';
            ...
            if (!this.codeViewMode) {
                ...
                let uline = (mxUtils.getValue(state.style, mxConstants.STYLE_FONTSTYLE, 0) &
        ...
        mxCellEditor.prototype.getBackgroundColor = function (state) {
            ...
            if (this.graph.getModel().isEdge(state.cell) || this.graph.getModel().isEdge(this.graph.getModel().getParent(state.cell))) {
                color = mxUtils.getValue(state.style, mxConstants.STYLE_LABEL_BACKGROUNDCOLOR, null);
         ...
         */
        mxGraphHandler.prototype.updateHint = function () {
            if (this.shape != null) {
                if (this.hint == null) {
         ...
         * Updates the hint for the current operation.
         */
        mxVertexHandler.prototype.updateHint = function () {
        ...
        mxVertexHandler.prototype.init = function () {
            ...
            this.editingHandler = mxUtils.bind(this, function () {
                this.redrawHandles();
            });
        ...
        mxVertexHandler.prototype.updateLinkHint = function (link, links) {
            ...
            else if (link != null || (links && links.length > 0)) {
                if (this.linkHint == null) {
