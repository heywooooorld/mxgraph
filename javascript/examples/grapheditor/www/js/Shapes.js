...
(function () {
    ...
    HandJiggle.prototype.lineTo = function (endX, endY) {
        ...
        if (this.lastX != null && this.lastY != null) {
            ...
            let letiation = this.defaultVariation;
            ...
            for (let s = 0; s < segs; s++) {
                ...
                let offset = (Math.random() - 0.5) * letiation;
    ...
    mxPerimeter.LifelinePerimeter = function (bounds, vertex, next) {
        ...
        if (!vertex) {
            ...
            sw = (parseFloat(vertex.style[mxConstants.STYLE_STROKEWIDTH] || 1) * vertex.view.scale / 2) - 1;
        }
    ...
    mxPerimeter.OrthogonalPerimeter = function (bounds, vertex, next) {
        let orthogonal = true;
        arguments[3] = orthogonal;
    ...
    mxPerimeter.StepPerimeter = function (bounds, vertex, next, orthogonal) {
        ...
        if (!vertex) {
            ...
            size = mxUtils.getValue(vertex.style, 'size', (fixed) ? StepShape.prototype.fixedSize : StepShape.prototype.size);
        }
        let x = bounds.x;
        let y = bounds.y;
        let w = bounds.width;
        ...
        let direction = (!vertex) ? mxUtils.getValue(
            vertex.style, mxConstants.STYLE_DIRECTION,
            mxConstants.DIRECTION_EAST) : mxConstants.DIRECTION_EAST;
        let points;
    ...
    function circleMarker(c, shape, type, pe, unitX, unitY, size, source, sw, filled) {
        let actualSize = size + sw;

        let pt = pe.clone();

        pe.x -= unitX * (2 * actualSize + sw);
        pe.y -= unitY * (2 * actualSize + sw);

        unitX = unitX * (actualSize + sw);
        unitY = unitY * (actualSize + sw);

        return function () {
            c.ellipse(pt.x - unitX - actualSize, pt.y - unitY - actualSize, 2 * actualSize, 2 * actualSize);
