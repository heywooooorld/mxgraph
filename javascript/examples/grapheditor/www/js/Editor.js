...
Editor.prototype.editAsNew = function (xml, title) {
    ...
    else if (typeof window.postMessage !== 'undefined' && (document.documentMode == null || document.documentMode >= 10)) {
...
Editor.prototype.readGraphState = function (node) {
    ...
    let ps = node.getAttribute('pageScale');
    ...
    let pw = node.getAttribute('pageWidth');
    let ph = node.getAttribute('pageHeight');
...
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll) {
    ...
    let dh = (document.documentElement.clientHeight > 0) ? document.documentElement.clientHeight :
        Math.max(document.body.clientHeight || 0, document.documentElement.clientHeight);
    let left = Math.max(1, Math.round((document.body.clientWidth - w - 64) / 2));
    ...
    let div = editorUi.createDiv('geDialog');
...
PrintDialog.prototype.create = function (editorUi) {
    ...
    mxUtils.write(span2, ' ' + mxResources.get('posterPrint') + ':');
    td.appendChild(span2);

    mxEvent.addListener(span2, 'click', function (evt) {
...
(function () {
    ...
    mxGraph.prototype.updatePageBreaks = function (visible, width, height) {
        ...
        let bounds2 = this.view.getBackgroundPageBounds();
